...
;; sources: https://github.com/scicloj/wolframite/
;;
;; It contains the following licence:
;;
;;
;;
;;
...
(defn view-no-summary* [form folded?]
  (let [wl-str (wl/->wl form {:output-fn str})
        input-img    (.evaluateToImage (proto/kernel-link (jlink-instance/get)) wl-str 0 0 0 true)
        b64img (bytes->b64encodedString input-img)]
    (kind/hiccup
...
(defn view-no-summary
  ...
  including the image as a file."
  ([form]
   (view-no-summary* form nil))
  ([form {:keys [folded?]}]
   (-> form
       (view-no-summary* folded?)
       kind/hiccup)))
